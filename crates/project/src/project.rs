...
use client::{
    Client, Collaborator, TypedEnvelope, UserStore, proto,
};
use clock::ReplicaId;
...
use rpc::{
    ...
    proto::LanguageServerPromptResponse,
};
...
impl Project {
    ...
    #[inline]
    pub fn unshare(&mut self, _cx: &mut Context<Self>) -> Result<()> {
        Ok(())
    }
    ...
    pub fn set_role(&mut self, _role: proto::ChannelRole, _cx: &mut Context<Self>) {
    }
    ...
    pub fn download_file(
        ...
        Task::ready(Err(anyhow!("download_file is not supported for local projects")))
    }
    ...
    pub fn open_server_settings(&mut self, _cx: &mut Context<Self>) -> Task<Result<Entity<Buffer>>> {
    ...
    /// Always returns the path unchanged for local projects.
    ...
    async fn handle_update_buffer(
        ...
        let buffer_store = this.read_with(&cx, |this, _cx| {
            this.buffer_store.clone()
        });
        BufferStore::handle_update_buffer(buffer_store, envelope, cx).await
    }
